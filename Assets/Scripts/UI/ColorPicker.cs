...
public class ColorPicker : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    private void Awake()
    {
        ...
        if (_mainCamera != null)
        {
            // Pobieramy początkowy kolor tła z głównej kamery
            Color initialColor = CameraManager.BackgroundColor;

            // Konwertujemy kolor tła kamery na HSV i ustawiamy jako początkowy kolor pickera
            Color.RGBToHSV(initialColor, out h, out s, out v);
        }
        else
        {
            h = s = v = 0; // Domyślnie ustawiamy czarny kolor
        }
